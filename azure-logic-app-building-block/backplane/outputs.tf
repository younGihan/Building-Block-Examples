...
output "gmail_connection_authorize_note" {
  value = var.email_connector == "gmail" ? "Before the workflow can send mail, authorize the API connection once: Azure Portal > Resource groups > ${azurerm_resource_group.this.name} > ${azurerm_api_connection.gmail[0].name} > 'Edit API connection' > sign in with the Google account and consent > Save. This one-time OAuth consent cannot be done via Terraform/OpenTofu." : null
}
